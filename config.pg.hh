...
// Generated by the protogen 0.6.1 compiler <https://github.com/brunexgeek/protogen>
...
#undef PROTOGEN_NS
#define PROTOGEN_NS protogen_0_6_1
    ...
    #define PROTOGEN_REG(err,input,msg) \
        do { if (err == NULL || err->set) return false; \
             ...
             return false; } while(false)
#endif
#ifndef PROTOGEN_BASE_0_6_1
#define PROTOGEN_BASE_0_6_1
...
public:
	...
	static const uint32_t PROTOGEN_VERSION = 0x000601;
	...
	bool deserialize( PROTOGEN_NS::InputStream<T> &in, bool required = false, PROTOGEN_NS::ErrorInfo *err = NULL ) {
		...
		while (true) {
			...
			if (name == PROTOGEN_FN_binding) {
				if (!this->binding().deserialize(in, required, err)) PROTOGEN_REV(err, in, name, "Binding");
				if (!PROTOGEN_NS::json::next(in)) PROTOGEN_REI(err, in, name);
				hfld[1] = true;
			}
			else
		...
		if (required) {
			...
			if (!hfld[2]) PROTOGEN_REM(err, in, "demonize");
