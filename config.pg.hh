...
// Generated by the protogen 0.6.0 compiler <https://github.com/brunexgeek/protogen>
// Source: /media/dados/projetos/dns-blocker/config.proto

#ifndef GUARD__media_dados_projetos_dns_blocker_config_proto
#define GUARD__media_dados_projetos_dns_blocker_config_proto
...
#undef PROTOGEN_NS
#define PROTOGEN_NS protogen_0_6_0
    ...
    #define PROTOGEN_REG(err,input,msg) \
        do { if (err == NULL || err->set) return false; \
             ...
             return false; } while(false)
#endif
#ifndef PROTOGEN_BASE_0_6_0
#define PROTOGEN_BASE_0_6_0
...
public:
	...
	static const uint32_t PROTOGEN_VERSION = 0x000600;
	...
	bool deserialize( PROTOGEN_NS::InputStream<T> &in, bool required = false, PROTOGEN_NS::ErrorInfo *err = NULL ) {
		...
		while (true) {
			...
			if (name == PROTOGEN_FN_address) {
				std::string value;
				if (!PROTOGEN_NS::traits<std::string>::read(in, value)) PROTOGEN_REV(err, in, name, "string");
				this->address(value);
			...
			if (name == PROTOGEN_FN_targets) {
				if (!PROTOGEN_NS::traits< std::vector<std::string> >::read(in, this->targets())) PROTOGEN_REV(err, in, name, "repeated string");
				if (!PROTOGEN_NS::json::next(in)) PROTOGEN_REI(err, in, name);
				hfld[1] = true;
			}
			else
		...
		if (required) {
			if (!hfld[0]) PROTOGEN_REM(err, in, "address");
			if (!hfld[1]) PROTOGEN_REM(err, in, "targets");
		}
		return true;
	}
	void clear() {
...
public:
	...
	#undef PROTOGEN_FN_bind_address
	#define PROTOGEN_FN_bind_address "bind_address"
	#undef PROTOGEN_FN_demonize
	#define PROTOGEN_FN_demonize "demonize"
	...
	static const int BIND_ADDRESS_NO = 3;
	PROTOGEN_NS::Field<std::string> bind_address;
	static const int DEMONIZE_NO = 4;
	PROTOGEN_NS::Field<bool> demonize;
	...
	Configuration(Configuration &&that) {
		...
		this->port = that.port;
		this->bind_address = that.bind_address;
		this->demonize = that.demonize;
	...
	void swap(Configuration &that) {
		...
		this->bind_address.swap(that.bind_address);
		this->demonize.swap(that.demonize);
	...
	bool undefined() const {
		return
			...
			this->port.undefined() &&
			this->bind_address.undefined() &&
			this->demonize.undefined() &&
	...
	void serialize( std::ostream &out ) const {
		...
		// bind_address
		if (!this->bind_address.undefined()) PROTOGEN_NS::json::write(out, first, PROTOGEN_FN_bind_address, this->bind_address());
		// demonize
		if (!this->demonize.undefined()) PROTOGEN_NS::json::write(out, first, PROTOGEN_FN_demonize, this->demonize());
		// blacklist
	...
	bool deserialize( PROTOGEN_NS::InputStream<T> &in, bool required = false, PROTOGEN_NS::ErrorInfo *err = NULL ) {
		...
		while (true) {
			...
			if (name == PROTOGEN_FN_external_dns) {
				if (!PROTOGEN_NS::traits< std::vector< ::NameServer> >::read(in, this->external_dns())) PROTOGEN_REV(err, in, name, "repeated NameServer");
				if (!PROTOGEN_NS::json::next(in)) PROTOGEN_REI(err, in, name);
				hfld[0] = true;
			}
			else
			...
			if (name == PROTOGEN_FN_port) {
				uint32_t value;
				if (!PROTOGEN_NS::traits<uint32_t>::read(in, value)) PROTOGEN_REV(err, in, name, "uint32");
				this->port(value);
				if (!PROTOGEN_NS::json::next(in)) PROTOGEN_REI(err, in, name);
				hfld[1] = true;
			}
			...
			// bind_address
			if (name == PROTOGEN_FN_bind_address) {
				std::string value;
				if (!PROTOGEN_NS::traits<std::string>::read(in, value)) PROTOGEN_REV(err, in, name, "string");
				this->bind_address(value);
				if (!PROTOGEN_NS::json::next(in)) PROTOGEN_REI(err, in, name);
				hfld[2] = true;
			}
			else
			// demonize
			if (name == PROTOGEN_FN_demonize) {
				bool value;
		...
		if (required) {
			if (!hfld[0]) PROTOGEN_REM(err, in, "external_dns");
			if (!hfld[1]) PROTOGEN_REM(err, in, "port");
			if (!hfld[2]) PROTOGEN_REM(err, in, "bind_address");
			if (!hfld[3]) PROTOGEN_REM(err, in, "demonize");
			if (!hfld[4]) PROTOGEN_REM(err, in, "blacklist");
		}
		return true;
	}
	void clear() {
...
#undef PROTOGEN_REM
#undef PROTOGEN_REG
#endif // GUARD__media_dados_projetos_dns_blocker_config_proto
